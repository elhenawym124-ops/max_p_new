...
const VolumeDiscounts: React.FC<VolumeDiscountsProps> = ({ productId }) => {
  ...
  const fetchDiscounts = async () => {
    try {
      ...
      const response = await axios.get(
        `${apiUrl}/products/${productId}/volume-discounts`,
        {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingDiscount) {
        // Update
        await axios.put(
          `${apiUrl}/products/${productId}/volume-discounts/${editingDiscount.id}`,
          formData,
          {
            headers: { Authorization: `Bearer ${token}` }
          }
  ...
  const handleDelete = async (discountId: string) => {
    ...
    try {
      ...
      const apiUrl = getApiUrl();
      await axios.delete(
        `${apiUrl}/products/${productId}/volume-discounts/${discountId}`,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
