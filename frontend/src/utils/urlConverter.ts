...
export const convertToPublicUrl = (url: string): string => {
  ...
  if (url.includes('https://www.mokhtarelhenawy.online')) {
    // استخدام الإعدادات من نظام البيئة
    const ngrokUrl = import.meta.env['VITE_NGROK_URL'] || 
                     'https://www.mokhtarelhenawy.online'; // رابط احتياطي
    
    const convertedUrl = url.replace('https://www.mokhtarelhenawy.online', ngrokUrl);
...
export const processImageMessage = (message: any) => {
  ...
  // معالجة رابط المحتوى
  if (processedMessage.content && processedMessage.content.includes('https://www.mokhtarelhenawy.online')) {
    processedMessage.content = convertToPublicUrl(processedMessage.content);
  }
  
  // معالجة fileUrl
  if (processedMessage.fileUrl && processedMessage.fileUrl.includes('https://www.mokhtarelhenawy.online')) {
...
export const getImageUrl = (message: any): string => {
  ...
  if (imageUrl) {
    ...
    if (imageUrl.startsWith('https://www.mokhtarelhenawy.online/') || 
